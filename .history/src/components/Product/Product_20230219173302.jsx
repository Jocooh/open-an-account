...
function Product({
  ...
  console.log(
    "selectedProductId[0].fin_prdt_nm :>> ",
    selectedProductId[0].fin_prdt_nm
  );
  ...
  return (
    <Wrapper>
        ...
        selectedProductRate ? (
          ...
          <TotalCost>
            {inputNum +
              Math.round(
                inputNum *
                  Math.pow(1 + (Number(selectedProductRate2) * 0.01) / 12, 12) -
                  inputNum
              ) -
              Math.round(
                Math.round(
      ...
      <ProductBox>
        <Name>
          <Prdt_nm>{selectedProductId[0].fin_prdt_nm}</Prdt_nm>
          <BsFillBookmarkFill
            onClick={() => {
              setScrap(true);
            }}
        ...
        <Info>
          <div>{selectedProductId[0].kor_co_nm}</div>
          <div>
        ...
        <Message>
          <li>가입 방법: {selectedProductId[0].join_way}</li>
          <li>가입 대상: {selectedProductId[0].join_member}</li>
          <li>
            (유의사항)
            {selectedProductId[0].etc_note}
          </li>
        </Message>
      </ProductBox>
    </Wrapper>
